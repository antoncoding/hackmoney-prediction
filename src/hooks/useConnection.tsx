...
export const useConnection = () => {
  ...
  const [web3, setWeb3] = useState<Web3>(new Web3(`https://kovan.infura.io/v3/${INFURA_KEY}`))

  // the network id that should be used to retrive data. (always a supported network)
  const [networkId, setNetworkId] = useState<SupportedNetworks>(42)

  // the network id that the current provider is on.
  const [currentProviderNetwork, setCurrentProvideNetwork] = useState<number|undefined>(42)
