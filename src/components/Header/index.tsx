...
export function Header({ primary, secondary, title }: { primary?: any; secondary?: any; title?: string }) {
  ...
  console.log(`currentProviderNetwork`, currentProviderNetwork)
  useEffect(() => {
    if (title) {
  ...
  return (
    <div>
      ...
      {currentProviderNetwork && !(currentProviderNetwork in SupportedNetworks) && (
        <Info mode="warning"> You're at the wrong network. Please switch to Kovan testnet </Info>
      )}
    </div>
  )
}
